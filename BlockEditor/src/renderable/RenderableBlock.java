...
public class RenderableBlock extends JComponent implements SearchableElement,
	...
	// /////////////////////////
	// Sockets and Labels
	/** Documentation does not exist for these components. Consult author */
	 ...
	 */
	private RenderableBlock(WorkspaceWidget parent, Long blockID,
			boolean isLoading) {
		super();
		/*
		...
		dragHandler = new JComponentDragHandler(this); // set up drag handler
														// delegate
		...
		this.blockLabel = new NameLabel(getBlock().getBlockLabel(),
		...
		this.footerLabel = new FooterLabel(getBlock().getFooterLabel(),
				BlockLabel.Type.FOOTER_LABEL, getBlock().isLabelEditable(),
				blockID);
		this.pageLabel = new PageLabel(getBlock().getPageLabel(),
		...
		// arrenged by sakai lab 2011/11/19
		else if (getBlock().isAbstractionBlock()) {
			blockShape = new SAbstractionBlockShape(this);
		} else {
	...
	private Dimension calcStackDimensions(RenderableBlock rb) {
		if (rb.getBlock().getAfterBlockID() != Block.NULL) {
			Dimension dim = calcStackDimensions(RenderableBlock
	 ...
	 * labels) with the associated Block's list of sockets.
	 * 
	 ...
	 * @complexity Running time for n Block sockets and m Renderable tags:
	 *             O(m+nm)=O(nm)
	 */
			...
			if (plugBlock != null) {
				if (plugBlock.getConnectorTo(blockID) == null) {
					throw new RuntimeException("one-sided connection from "
							+ getBlock().getBlockLabel() + " to "
							+ Block.getBlock(blockID).getBlockLabel());
				}
				RenderableBlock.getRenderableBlock(plug.getBlockID())
	...
	/**
	 * created by sakai lab 2011/11/18
	 * 
	 * @return blockLabel
	 */
	protected NameLabel getBlockLabel() {
		return blockLabel;
	}
	 ...
	 * @return headerLabel
	 */
	protected HeaderLabel getHeaderLabel() {
		return headerLabel;
	}
	 ...
	 * @return footerLabel
	 */
	protected FooterLabel getFooterLabel() {
		return footerLabel;
	}
	 ...
	 */
	/*
	 * public void linkDefArgs(){ if(!linkedDefArgsBefore &&
	 * getBlock().hasDefaultArgs()){ Iterator<Long> ids =
	 * getBlock().linkAllDefaultArgs().iterator(); Long id;
	 * while(ids.hasNext()){ id = ids.next(); if(!Block.NULL.equals(id)){
	 * RenderableBlock arg = new RenderableBlock(this.getParentWidget(), id);
	 * getParentWidget().addBlock(arg);
	 * Workspace.getInstance().notifyListeners(new
	 * WorkspaceEvent(getParentWidget(), arg.getBlockID(),
	 * WorkspaceEvent.BLOCK_ADDED, true)); } } this.moveConnectedBlocks();
		...
		if (!linkedDefArgsBefore && getBlock().hasDefaultArgs()) {
			...
			for (int i = 0; i < size; i++) {
				Workspace.getInstance().notifyListeners(
						new WorkspaceEvent(this.getParentWidget(), argList.get(
								i).getBlockID(), WorkspaceEvent.BLOCK_ADDED,
								true));

				// must call this method to update the dimensions of this
				// ria in the future would be good to just link the default args
	...
	/**
	 * Returns the Renderable specified by blockID; null if RenderableBlock does
	 * not exist
	 * 
	 * @param blockID
	 *            the block id of the desired RenderableBlock
	 * @return the Renderable specified by blockID; null if RenderableBlock does
	 *         not exist
	 */
	...
	private void updateSocketSpace(BlockConnector connectedSocket,
		...
		if (!isConnected) {
			...
			if (getBlock().getBeforeBlockID() != Block.NULL
				...
				BlockConnector beforeSocket = Block.getBlock(beforeID)
						.getConnectorTo(getBlockID());
				RenderableBlock.getRenderableBlock(beforeID).updateSocketSpace(
						beforeSocket, getBlockID(), true);
				return;
			}

			// if empty before socket, then return
			// if(getBlock().hasBeforeConnector() &&
			// getBlock().getBeforeBlockID() == Block.NULL) return;
		...
		if (plugEquiv != null && plugEquiv.hasBlock()) {
			...
			BlockConnector socketEquiv = Block.getBlock(plugID).getConnectorTo(
					getBlockID());
			// update the socket space of a connected before/parent block
			RenderableBlock.getRenderableBlock(plugID).updateSocketSpace(
					socketEquiv, getBlockID(), true);
		}
	}
	...
	private Dimension calcDimensionOfSocket(BlockConnector socket) {
		...
		while (curBlockID != Block.NULL) {
			...
			RenderableBlock curRenderableBlock = RenderableBlock
					.getRenderableBlock(curBlockID);// curRenderableblockがnullになる　ID
													// 1004,
			...
			if (curBlock.getNumSockets() > 0 && !curBlock.isInfix()) {
				...
				if (maxSocWidth > 0) {
					...
					// counted twice
					maxSocWidth -= BlockConnectorShape.getDataPlugWidth(socket);
	...
	private void reformBlockShape() {
		...
		// for zooming, create an AffineTransform to scale the block shape
		...
		// ////////////////////////////////////////
		// arranged by sakai lab 2011/11/18
		if (headerLabel != null) {
			headerLabel.update();
		}
		// arranged by sakai lab 2011/11/22
	 ...
	 * 
	 * @param buffImgG2
	 *            the current Graphics2D representation of this
	 * @param width
	...
	private void repositionBlockImages(int width, int height) {
		...
		// need to take other images into acct if we enable multiple block
		// images
	...
	public void moveConnectedBlocks() {
		...
		for (BlockConnector socket : BlockLinkChecker.getSocketEquivalents(b)) {
			...
			if (socket.hasBlock()) {
				...
				plugLocation = rb
						.getSocketPixelPoint(BlockLinkChecker
								.getPlugEquivalent(Block.getBlock(socket
										.getBlockID())));
		...
		if (SwingUtilities.isLeftMouseButton(e)) {
			...
			if (!pickedUp) {
				if (SBlockEditor.DEBUG) {
					// #matsuzawa こういう事もあります．
					// throw new
					// RuntimeException("dropping without prior dragging?");
			...
			if (dragging) {
				...
				// otherwise, if a link WAS found...
					...
					if (blockID.equals(link.getSocketBlockID())) {
						...
						// parent.
						widget = getRenderableBlock(link.getPlugBlockID())
								.getParentWidget();
					} else {
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (SwingUtilities.isLeftMouseButton(e)) {
			if (!pickedUp) {
				if (SBlockEditor.DEBUG) {
					// #matsuzawa プルダウンメニューを出したままドラッグ，こういう事もあります．
					// throw new
					// RuntimeException("dragging without prior pickup?");
	...
	/*
	 * private void showBlockDetailForDebug() { System.out.println("blockID:" +
	 * blockID); System.out.println("genus:" + getGenus());
	 ...
	 */
	public String getSaveString() {// #ohata replaced
		if (comment != null)
			/*
	...
	public static RenderableBlock loadBlockNode(Node blockNode, WorkspaceWidget parent, HashMap<Long, Long> idMapping) {
		...
		if (isBlock || isBlockStub) {
			RenderableBlock rb = new RenderableBlock(parent, Block
					.loadBlockFrom(blockNode, idMapping).getBlockID(), true);
			...
			if (rb.getBlock().labelMustBeUnique()) {
				// check the instance number of this block
	...
	public void setZoomLevel(double newZoom) {
		...
		// arranged by sakai lab 2011/11/18
		if (headerLabel != null && getBlock().hasHeaderLabel()) {
			this.headerLabel.setZoomLevel(newZoom);
		}
		// arranged by sakai lab 2011/11/22
		if (footerLabel != null && getBlock().hasHeaderLabel()) {
			this.footerLabel.setZoomLevel(newZoom);
		}
		if (pageLabel != null && getBlock().hasPageLabel()) {
	...
	/**
	 * created by sakai lab 2011/11/30
	 */
	public void initUpdateCollapse() {
		if (collapseLabel != null) {
	...
	/**
	 * returns the larger width of the ProcedureCollapseLabel or CommentLabel if
	 * they exist for this RenderableBlock
	 * 
	 * @return
	 */
