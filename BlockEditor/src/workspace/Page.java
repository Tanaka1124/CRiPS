 ...
 * panel that renders its collection of blocks. Abstractly, a page has seven
 ...
 * depending on the zoom level and minimumPixelWidth. Using the zoom level, it
 * tries to maintain a constant aspect ratio but the absolute sizes varies with
 * a bigger/smaller zoom level. the minimumPixelWidth limits the width from
 * going below a certain size, no matter what the system tries to set the
...
public class Page implements WorkspaceWidget, SearchableContainer,
	...
	/**
	 * The GUI component for interfacing with the user to help the user collapse
	 * or restore the page
	 */
	 ...
	 * @param pageWidth
	 *            - the abstract width of this page (this.width)
	 * @param pageHeight
	 ...
	 *          "pageWidth". If "pageWidth is <= to zero, then set the width to
	 *          the DEFAULT_ABSTRACT_WIDTH. 3) The abstract height of this page
	 *          equals DEFAULT_ABSTRACT_HEIGHT. 4) The drawer name equals
	 *          pageDrawer if and only if Workspace.everyPageHasDrawer==true. 5)
	 *          The color of this page is null. 6) The font of this page is
	 *          "Default", PLAIN, and 12. 7) The set of blocks is empty.
	 */
	public Page(String name, int pageWidth, int pageHeight, String pageDrawer) {
		this(name, pageWidth, pageHeight, pageDrawer, true, null, null);
	}

	public Page(String name, int pageWidth, int pageHeight, String pageDrawer,
			boolean inFullview, Color defaultColor, String superClass) {
		super();
		this.defaultColor = defaultColor;
		this.pageJComponent.setLayout(null);
		this.pageJComponent.setName(name);
		this.abstractWidth = pageWidth > 0 ? pageWidth
				: Page.DEFAULT_ABSTRACT_WIDTH;
	 ...
	 * @effects constructs a new Page such that: 1) The name of this page equals
	 *          the argument "name". 2) The abstract width of this page equals
	 *          DEFAULT_ABSTRACT_WIDTH. 3) The abstract height of this page
	 *          equals DEFAULT_ABSTRACT_HEIGHT. 4) The drawer name equals "name"
	 *          5) The color of this page is null. 6) The font of this page is
	 *          "Default", PLAIN, and 12. 7) The set of blocks is empty.
	 */
	 ...
	 *          DEFAULT_ABSTRACT_WIDTH. 3) The abstract height of this page
	 *          equals DEFAULT_ABSTRACT_HEIGHT. 4) The drawer name equals "" 5)
	 *          The color of this page is null. 6) The font of this page is
	 *          "Default", PLAIN, and 12. 7) The set of blocks is empty.
	 */
	...
	/**
	 * THIS METHOD NOT YET DOCUMENTED OR IMPLEMENTED Removes all the
	 * RenderableBlock content of this. Called when the Workspace is being
	 * reset. Does not fire block removed events.
	 */
	public void reset() {
	...
	/**
	 * Destructs this Page by setting its set of blocks to empty. Does NOT fire
	 * block removed events.
	 */
	...
	/**
	 * Sets the page id. Consider the page id "final" but settable - once set,
	 * it cannot be modified or unset.
	 */
	...
	public Collection<RenderableBlock> getTopLevelBlocks() {
		...
		for (RenderableBlock renderable : this.getBlocks()) {
			...
			if (block.getPlug() == null || block.getPlugBlockID() == null
	 ...
	 *          sets the color to the deafult gray.
	 */
	 ...
	 *          the zoom level. May need to convert form pixel to abstract
	 *          b2j.model.
	 */
	 ...
	 * @modifies this.width
	 * @effects sets abstract width to pixelWidth taking into account the zoom
	 *          level. May need to convert form pixel to abstract b2j.model.
	 */
	 ...
	 *          abstract width and height with the graphical view. This process
	 *          includes moving this page's JComponent to (pixelXCor,0) and
	 *          setting this page's JComponent size to (this.abstractwidth*zoom,
	 *          this.abstractheight*zoom)
	 */
	 ...
	 *          page to fit the new block. It must then notify listeners that
	 *          the page's size may have changed
	 */
	public void reformBlockPosition(RenderableBlock block) {
		//move blocks in
		Point p = SwingUtilities.convertPoint(block.getParent(),
		...
		if (p.x < block.getHighlightStrokeWidth() / 2 + 1) {
			...
			p = SwingUtilities.convertPoint(block.getParent(),
					block.getLocation(), this.pageJComponent);
		} else if (p.x + block.getWidth() + block.getHighlightStrokeWidth() / 2
				+ 1 > this.pageJComponent.getWidth()) {
			this.setPixelWidth(p.x + block.getWidth()
		...
		if (block.hasComment()) {
			...
			if (p.x + block.getComment().getWidth() + 1 > this.pageJComponent
					.getWidth()) {
	 ...
	 * @ffects sets the minimumPixelWidth such that the following condition
	 *         holds: DEFAULT_MINIMUMWIDTH < new minimumPixelWidth && for each
	 *         block, b, in this page's set of blocks { b.x+b.width < new
	 *         minimumPixelWidth}
	 */
	...
	/**
	 * @requires the current set of blocks of this page != null (though it may
	 *           be empty)
	 ...
	 *          delaying graphicalupdates until all of the blocks have been
	 *          added.
	 * @ovverride WorkspaceWidget.blockEntered()
	 */
	...
	public ArrayList<RenderableBlock> loadPageFrom(Node pageNode,
	...
	public void addLoadedBlocks(Collection<RenderableBlock> loadedBlocks,
		...
		for (RenderableBlock rb : loadedBlocks) {
			if (rb != null) {
				...
				//TODO 抽象化ブロックもLoad時点で閉じることができるようにした。
				if (rb.isCollapsed()) {
	...
	public String getSaveString() {
		...
		if (superClass != null) {
			appendAttribute("page-superClass", superClass, buf);
		}
		if (pageId != null) {
	...
	/**
	 * a data structure that holds the name, width, color, set of blocks, and
	 * set of renderable blocks in this page.
	 */
...
/**
 * This class serves as the zoomable JComponent and RBParent of the page that
 * wraps it.
 */
class PageJComponent extends JLayeredPane implements RBParent {
	...
	public void paintComponent(Graphics g) {
		...
		//set label color
		if (this.getBackground().getBlue() + this.getBackground().getGreen()
				+ this.getBackground().getRed() > 400) {
		...
		if (fullview) {
			int xpos = (int) (this.getWidth() * 0.5 - g.getFontMetrics()
					.getStringBounds(this.getName(), g).getCenterX());
			...
			g.drawImage(this.getImage(), imageX, getHeight() / 4 + 5,
					imageWidth, imageWidth, null);
...
/**
 * A BlockStatckSortUtil is a utilities class that serves to order blocks from
 * closest to furthest blocks (relative to the x=0 axis).
 */
class BlockStackSorterUtil {
	...
	private static final Rectangle positioningBounds = new Rectangle(
			BUFFER_BETWEEN_BLOCKS, BUFFER_BETWEEN_BLOCKS, 0, 0);
	/**
	 * An ordered set of blocks. Blocks are ordered from closest to furthest
	 ...
	 *          the page
	 */
	protected static void sortBlockStacks(Page page,
			Collection<RenderableBlock> topLevelBlocks) {
		blocksToArrange.clear();
		positioningBounds.setBounds(BUFFER_BETWEEN_BLOCKS,
				BUFFER_BETWEEN_BLOCKS, 0, BUFFER_BETWEEN_BLOCKS);
		...
		for (RenderableBlock block : blocksToArrange) {
			...
			positioningBounds.width = Math.max(positioningBounds.width,
					bounds.width);
			positioningBounds.height = positioningBounds.height + bounds.height
